...
static UBYTE g_screen_title[80] = "Master of Magic 0.2.1";
...
/* Zrzut CALEGO ekranu Amigi (z paskiem tytulowym Intuition) do pliku PPM.
 * Robiony od srodka goscia, bo zrzut okna WinUAE na hoscie nie jest wiarygodny:
 * PrintWindow przy Direct3D oddaje stary kadr, a kopia z ekranu pulpitu zlapie
 * to, co akurat przykrywa okno (2026-09-18: prywatne okno developera).
 *
 * Czyta piksele przez ReadPixelArray8 z RastPortu ekranu (albo okna w trybie
 * WB) i barwi je paleta z ColorMap tego ekranu. Wolne (kilkaset ms), ale
 * uzywane tylko na zadanie - "snap" z Work:autoinput.txt.
 * Zwraca 0, gdy plik powstal. */
int amigagfx_dump_screen(const char *path)
{
	struct Screen *scr = (g_screen != NULL) ? g_screen
	                   : ((g_window != NULL) ? g_window->WScreen : NULL);
	struct RastPort *rp;
	...
	FILE *f;
	int w, h, x, y;
	UBYTE r8[256], g8[256], b8[256];
	ULONG rgb[3];
	int kolorow;

	if (scr == NULL || path == NULL) return 1;
	rp = &scr->RastPort;
	w = (int)scr->Width;
	h = (int)scr->Height;
	if (w <= 0 || h <= 0 || w > 4096 || h > 4096) return 1;

	row = (UBYTE *)AllocVec((ULONG)((w + 15) & ~15), MEMF_ANY);
	tempbm = AllocBitMap((ULONG)((w + 15) & ~15), 1UL,
	                     (ULONG)GetBitMapAttr(rp->BitMap, BMA_DEPTH), 0UL, rp->BitMap);
	if (row == NULL || tempbm == NULL) {
		FreeVec(row);
		if (tempbm != NULL) FreeBitMap(tempbm);
		return 1;
	}
	InitRastPort(&temprp);
	...
	kolorow = (cm != NULL) ? (int)cm->Count : 0;
	if (kolorow > 256) kolorow = 256;
	for (x = 0; x < 256; x++) {
		if (x < kolorow) {
			GetRGB32(cm, (ULONG)x, 1UL, rgb);
			r8[x] = (UBYTE)(rgb[0] >> 24);
			g8[x] = (UBYTE)(rgb[1] >> 24);
			b8[x] = (UBYTE)(rgb[2] >> 24);
		} else {
	...
	fprintf(f, "P6\n%d %d\n255\n", w, h);
	for (y = 0; y < h; y++) {
		ReadPixelArray8(rp, 0, (UWORD)y, (UWORD)(w - 1), (UWORD)y, row, &temprp);
		for (x = 0; x < w; x++) {
			UBYTE c = row[x];
			fputc(r8[c], f);
			fputc(g8[c], f);
	...
	FreeVec(row);
	FreeBitMap(tempbm);
	amigagfx_log("zrzut ekranu zapisany");
	return 0;
}

/* 1 = our screen is the frontmost one (IntuitionBase->FirstScreen), 0 = not,
 * -1 = no screen of our own (window mode / closed). Read without locking:
 * a diagnostic, not a decision. */
int amigagfx_screen_in_front(void)
{
	if (g_screen == NULL) return -1;
	return (IntuitionBase->FirstScreen == g_screen) ? 1 : 0;
}

/* Bring our screen in front of all others (no-op without one). */
void amigagfx_screen_to_front(void)
{
	if (g_screen != NULL) ScreenToFront(g_screen);
}
...
static int finish_window_open(void)
{
	...
	fprintf(stdout, "amiga: window open on the Workbench screen,"
	                " chunky %dx%d (pitch %d) - handing control to the game\n",
...
int amigagfx_open(int w, int h, int show_bar, int backend)
{
	...
	ActivateWindow(g_window);
	fprintf(stdout, "amiga: window open, IDCMP active - handing control to the game\n");
	return 0;
}
