...
/* 1 = our screen is frontmost, 0 = another screen is in front, -1 = no
 * screen of our own. amigagfx_screen_to_front() raises it. */
int amigagfx_screen_in_front(void);
void amigagfx_screen_to_front(void);

/* Zrzut calego ekranu Amigi (z paskiem tytulowym) do pliku PPM. 0 = zapisany.
 * Wolny - tylko na zadanie ("snap" z Work:autoinput.txt). */
